...
public class ModGame : IActionListener
{
    ...
    public static void HandleKeyPress(int keyCode)
    {
        ...
        else
        {
            ...
            if(keyCode == 106) //j
            {
                Enter(TileMap.vGo.elementAt(0) as Waypoint);
            }
            if(keyCode == 107) //k
            {
                Enter(TileMap.vGo.elementAt(2) as Waypoint);
            }
            if(keyCode == 108)
            {
                Enter(TileMap.vGo.elementAt(1) as Waypoint);
            }
            if (keyCode == 112) //p
            {
    ...
    public static void Enter(Waypoint waypoint)
    {
        int num = ((waypoint.maxX < 60) ? waypoint.maxX - 5 : ((waypoint.minX <= TileMap.pxw - 60) ? ((waypoint.minX + waypoint.maxX) / 2) : (TileMap.pxw - 15)));
